...
def head_open_url(url):
	...
	opener.addheaders = [('User-Agent','Mozilla/5.0 (Windows NT 5.1) AppleWebKit/535.11 (KHTML, like Gecko) Chrome/17.0.963.84 Safari/535.11 SE 2.X MetaSr 1.0')]
...
def get_top_url(top_name):
	...
	while 1:
		...
		try:
			...
			p = re.compile('href="([^"]*)"><em><!--red_beg-->' + top_name)
			#搜索到公众号并成功获取url
...
def read_main():	
	...
	for name in top_list:
		...
		with open('final_university.txt','a') as f:
	...
	if do_compare == 'yes':
		with open('final_university.txt','r') as f:
		...
		with open('compare_university.txt','w') as f:
			num = 1
			for each in campare_list:
				f.write('%s.'%num + each + '\n')
				num += 1
		print('获取比较数据功能正常,数据保存在与程序相同文件夹的compare_university.txt文件中')			
